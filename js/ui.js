...
function createLayerDiv(data) {
	...
	// create
	$("#layer_list" ).prepend( '<div class="img_list_div" id="' 
		+ data.items.id + '" data-map_id="' + data.items.id 
		+ '"><img src="'+W_LEAFLET.base_url+'/maps/thumb/'
		+data.items.id+'" /><span class="handle"> </span><div class="delete">X</div><div class="mapinfo">' 
		+ decodeURIComponent(data.items.title.replace(/_/g,' ')) + '</div><div class="slider"></div></div>' );
